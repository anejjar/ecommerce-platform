...
describe('account\orders\[id]\reorder', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostaccount\orders\[id]\reorder')

    // Test first available method
    const { GET } = await import('@/app/apiaccount\orders\[id]\reorder/route')
    if (GET) {
