...
describe('account\payment-methods\[id]\set-default', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostaccount\payment-methods\[id]\set-default')

    // Test first available method
    const { GET } = await import('@/app/apiaccount\payment-methods\[id]\set-default/route')
    if (GET) {
