...
describe('account\payment-methods\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostaccount\payment-methods\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiaccount\payment-methods\[id]/route')
    if (GET) {
