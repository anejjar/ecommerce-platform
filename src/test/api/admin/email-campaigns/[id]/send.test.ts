...
describe('admin\email-campaigns\[id]\send', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\email-campaigns\[id]\send')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\email-campaigns\[id]\send/route')
    if (GET) {
