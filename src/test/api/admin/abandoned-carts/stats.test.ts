...
describe('admin\abandoned-carts\stats', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\abandoned-carts\stats')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\abandoned-carts\stats/route')
    if (GET) {
