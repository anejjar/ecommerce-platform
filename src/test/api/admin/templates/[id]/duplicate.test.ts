...
describe('admin\templates\[id]\duplicate', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\templates\[id]\duplicate')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\templates\[id]\duplicate/route')
    if (GET) {
