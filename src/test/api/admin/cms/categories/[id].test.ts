...
describe('admin\cms\categories\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\cms\categories\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\cms\categories\[id]/route')
    if (GET) {
