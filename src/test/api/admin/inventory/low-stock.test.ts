...
describe('admin\inventory\low-stock', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\inventory\low-stock')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\inventory\low-stock/route')
    if (GET) {
