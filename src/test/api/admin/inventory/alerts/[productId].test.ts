...
describe('admin\inventory\alerts\[productId]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\inventory\alerts\[productId]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\inventory\alerts\[productId]/route')
    if (GET) {
