...
describe('admin\inventory\bulk-update', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\inventory\bulk-update')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\inventory\bulk-update/route')
    if (GET) {
