...
describe('admin\inventory\stock-history', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\inventory\stock-history')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\inventory\stock-history/route')
    if (GET) {
