...
describe('admin\inventory\reports', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\inventory\reports')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\inventory\reports/route')
    if (GET) {
