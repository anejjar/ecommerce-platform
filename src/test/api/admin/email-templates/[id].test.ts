...
describe('admin\email-templates\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\email-templates\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\email-templates\[id]/route')
    if (GET) {
