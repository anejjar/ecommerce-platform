...
describe('admin\import\[id]\process', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\import\[id]\process')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\import\[id]\process/route')
    if (GET) {
