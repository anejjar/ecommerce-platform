...
describe('admin\import\[id]\validate', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\import\[id]\validate')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\import\[id]\validate/route')
    if (GET) {
