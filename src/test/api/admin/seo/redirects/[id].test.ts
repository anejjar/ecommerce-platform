...
describe('admin\seo\redirects\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\seo\redirects\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\seo\redirects\[id]/route')
    if (GET) {
