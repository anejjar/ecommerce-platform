...
describe('admin\seo\schema-markup\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\seo\schema-markup\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\seo\schema-markup\[id]/route')
    if (GET) {
