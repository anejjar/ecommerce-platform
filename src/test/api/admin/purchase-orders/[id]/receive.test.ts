...
describe('admin\purchase-orders\[id]\receive', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\purchase-orders\[id]\receive')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\purchase-orders\[id]\receive/route')
    if (GET) {
