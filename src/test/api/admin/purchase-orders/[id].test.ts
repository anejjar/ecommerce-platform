...
describe('admin\purchase-orders\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\purchase-orders\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\purchase-orders\[id]/route')
    if (GET) {
