...
describe('admin\backup\[id]\download', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\backup\[id]\download')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\backup\[id]\download/route')
    if (GET) {
