...
describe('admin\backup\[id]\restore', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\backup\[id]\restore')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\backup\[id]\restore/route')
    if (GET) {
