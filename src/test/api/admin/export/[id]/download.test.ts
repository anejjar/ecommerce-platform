...
describe('admin\export\[id]\download', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\export\[id]\download')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\export\[id]\download/route')
    if (GET) {
