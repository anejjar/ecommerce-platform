...
describe('admin\categories\[id]\translations', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\categories\[id]\translations')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\categories\[id]\translations/route')
    if (GET) {
