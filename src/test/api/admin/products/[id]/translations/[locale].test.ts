...
describe('admin\products\[id]\translations\[locale]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\products\[id]\translations\[locale]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\products\[id]\translations\[locale]/route')
    if (GET) {
