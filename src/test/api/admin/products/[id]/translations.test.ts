...
describe('admin\products\[id]\translations', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\products\[id]\translations')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\products\[id]\translations/route')
    if (GET) {
