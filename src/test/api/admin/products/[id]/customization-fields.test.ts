...
describe('admin\products\[id]\customization-fields', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\products\[id]\customization-fields')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\products\[id]\customization-fields/route')
    if (GET) {
