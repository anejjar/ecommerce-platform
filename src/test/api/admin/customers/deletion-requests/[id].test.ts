...
describe('admin\customers\deletion-requests\[id]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\customers\deletion-requests\[id]')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\customers\deletion-requests\[id]/route')
    if (GET) {
