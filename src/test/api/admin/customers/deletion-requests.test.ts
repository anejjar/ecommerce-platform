...
describe('admin\customers\deletion-requests', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostadmin\customers\deletion-requests')

    // Test first available method
    const { GET } = await import('@/app/apiadmin\customers\deletion-requests/route')
    if (GET) {
