...
describe('products\[id]\variant-options', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostproducts\[id]\variant-options')

    // Test first available method
    const { GET } = await import('@/app/apiproducts\[id]\variant-options/route')
    if (GET) {
