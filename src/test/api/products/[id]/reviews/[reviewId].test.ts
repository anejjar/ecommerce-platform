...
describe('products\[id]\reviews\[reviewId]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostproducts\[id]\reviews\[reviewId]')

    // Test first available method
    const { GET } = await import('@/app/apiproducts\[id]\reviews\[reviewId]/route')
    if (GET) {
