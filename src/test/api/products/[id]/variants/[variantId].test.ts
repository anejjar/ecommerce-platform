...
describe('products\[id]\variants\[variantId]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostproducts\[id]\variants\[variantId]')

    // Test first available method
    const { GET } = await import('@/app/apiproducts\[id]\variants\[variantId]/route')
    if (GET) {
