...
describe('products\images\[imageId]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostproducts\images\[imageId]')

    // Test first available method
    const { GET } = await import('@/app/apiproducts\images\[imageId]/route')
    if (GET) {
