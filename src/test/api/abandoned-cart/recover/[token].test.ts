...
describe('abandoned-cart\recover\[token]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostabandoned-cart\recover\[token]')

    // Test first available method
    const { GET } = await import('@/app/apiabandoned-cart\recover\[token]/route')
    if (GET) {
