...
describe('cart\items\[id]\customizations', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhostcart\items\[id]\customizations')

    // Test first available method
    const { GET } = await import('@/app/apicart\items\[id]\customizations/route')
    if (GET) {
