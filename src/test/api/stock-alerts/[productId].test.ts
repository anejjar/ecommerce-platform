...
describe('stock-alerts\[productId]', () => {
  beforeEach(() => {
    resetAllMocks()
  })
  ...
  it('returns 401 for unauthorized requests', async () => {
    ...
    const req = new NextRequest('http://localhoststock-alerts\[productId]')

    // Test first available method
    const { GET } = await import('@/app/apistock-alerts\[productId]/route')
    if (GET) {
